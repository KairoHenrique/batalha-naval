...
Trabalho **individual**. Estado atual: Semana 2, **T3 concluído** (tabuleiro + navios + conferência). Menu, partidas, IA, stats, replay e GUI web ainda não entraram.
...
|----|-----------|------|--------|
| RF01 | Menu principal | `menu.py` | pendente (T4) |
...
| RF05 | Validar jogadas (limites e não repetidas) | `utils.py` (parse + RN02) | parse feito; loop de partida pendente |
| RF06 | Água, acerto, navio afundado | partida / `jogador.py` | pendente |
| RF07 | Fim: vencedor, jogadas, tempo | `utils.formatar_tempo` + tela de fim | cronômetro pronto; tela pendente |
| RF08 | Nova partida pelo menu | `menu.py` | pendente |
| RF09 | Jogador × Computador e Dois Jogadores | — | pendente |
...
Regras de negócio já cobertas no código atual: **RN01** (`parse_coordenada`) e rejeição de casa inválida. **RN02–RN05** entram com o loop de partida.
...
Próximos arquivos obrigatórios: `main.py`, `menu.py`, `jogador.py`, `computador.py`, `estatisticas.py`, `replay.py`. Extras planejados: `partida.py`, `api.py`, `web/` (Next.js).

## Implementação

Fluxo até o T3: coordenada → matriz → frota visível para conferência.
...
Não há `main.py` ainda. Cada módulo tem um `__main__` para inspeção:

```bash
python tabuleiro.py    # mockup 6.3 (X em B2, grande em D3–G3, O em H4)
python navios.py       # 40 frotas sem overlap + conferência C/R
```

A conferência limpa a tela, imprime o tabuleiro com `N` e a lista de coordenadas por navio. **C** confirma; **R** gera outra disposição. Entrada inválida não troca a frota.
...
# 3. Frota + conferência (RF03, RF04, RF10)
python navios.py
```
...
| Conferência | teclar `R` depois `C` | R redesenha; C encerra com “Frota confirmada” |
| Diário | `docs/diario.md` | T1–T3 preenchidos; T4–T11 em branco |
